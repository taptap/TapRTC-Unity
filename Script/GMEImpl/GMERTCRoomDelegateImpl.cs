...
namespace TapTap.RTC.GMEImpl
{
    public class GMERTCRoomDelegateImpl : ITapRTCRoomDelegate, ITapRTCRangeAudioCtrl
    {
        ...
        public override void Join(JoinRoomResponse response)
        {
            ...
            if (RangeAudio)
            {
                SetRangeAudioMode(RangeAudioMode);
                if (TeamId == 0)
                {
                    throw new TapRTCException(-1, "You must set team_id before enter team room!");
                }
            }
        ...
        public bool SetRangeAudioMode(RangeAudioMode mode)
        {
            ...
            bool result;
            if (RangeAudioMode == RangeAudioMode.TEAM)
            {
                result = context.SetRangeAudioMode(ITMGRangeAudioMode.ITMG_RANGE_AUDIO_MODE_TEAM) == 0;
            }
            else
            {
                result = context.SetRangeAudioMode(ITMGRangeAudioMode.ITMG_RANGE_AUDIO_MODE_WORLD) == 0;
            }
        ...
        public bool SetRangeAudioTeam(int teamId)
        {
            ...
            if (context.SetRangeAudioTeamID(teamId) != 0) return false;
            TeamId = teamId;
            return true;
        }
        ...
        public bool UpdateAudioReceiverRange(int range)
        {
            ...
            return context.GetRoom().UpdateAudioRecvRange(range) == 0;
        }
        ...
        public bool UpdateSelfPosition(Position position, Forward forward)
        {
            ...
            return context.GetRoom()
                .UpdateSelfPosition(new[] {position.x, position.y, position.z}, forward.asixForward, forward.asixRight,
                    forward.asixUp) == 0;
        }
